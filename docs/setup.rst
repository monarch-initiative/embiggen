...
xn2v is a Python3 package that implements an extension of the node2vec algorithm for heterogeneous networks.

Requirements
~~~~~~~~~~~~
xn2v has been tested with Python 3.6 and 3.7. It requires the following packages.
...
xn2v to analyze a heterogeneous network comprised of protein-protein
...
``data`` in the same directory in which ``run_xn2v.py`` is located.
...
You should not unpack these files. If you use a different directory to store the files, you will need to pass the path to this directory to the ``xn2vParser`` constructor (see TODO).
...
We are now ready to run xn2v (see :ref:`rstrunning`).
