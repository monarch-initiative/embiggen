...
HN2V is a Python3 package that implements an extension of the node2vec algorithm for heterogeneous networks.

Requirements
~~~~~~~~~~~~
HN2V has been tested with Python 3.6 and 3.7. It requires the following packages.
...
HN2V to analyze a heterogeneous network comprised of protein-protein
...
``data`` in the same directory in which ``run_hn2v.py`` is located.
...
You should not unpack these files. If you use a different directory to store the files, you will need to pass the path to this directory to the ``HN2VParser`` constructor (see TODO).
...
We are now ready to run HN2V (see :ref:`rstrunning`).
