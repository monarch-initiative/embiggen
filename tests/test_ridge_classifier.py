...
from embiggen.node_label_prediction.node_label_prediction_sklearn.ridge_classifier_node_label_prediction import (
    RidgeClassifierNodeLabelPrediction,
)
from embiggen.edge_prediction.edge_prediction_sklearn.ridge_classifier_edge_prediction import (
    RidgeClassifierEdgePrediction,
)
from embiggen.edge_label_prediction.edge_label_prediction_sklearn.ridge_classifier_edge_label_prediction import (
    RidgeClassifierEdgeLabelPrediction,
)
...
def test_ridge_classifier_node_label_prediction_smoke_test():
    ...
    for g, data in (
        ...
        model = RidgeClassifierNodeLabelPrediction().into_smoke_test()
...
def test_ridge_classifier_edge_prediction_smoke_test():
    ...
    for g, data in (
        ...
        if data is None:
            data = DegreeSPINE().into_smoke_test().fit_transform(g)
        model = RidgeClassifierEdgePrediction().into_smoke_test()
...
def test_ridge_classifier_edge_label_prediction_smoke_test():
    ...
    for g, data in ((binary, None),):
        if data is None:
            data = DegreeSPINE().into_smoke_test().fit_transform(g)
        model = RidgeClassifierEdgeLabelPrediction().into_smoke_test()
        model.fit(g, node_features=data, edge_features=HyperSketching())
        model.predict(g, node_features=data, edge_features=HyperSketching())
        model.predict_proba(g, node_features=data, edge_features=HyperSketching())
