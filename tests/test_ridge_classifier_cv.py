...
from embiggen.node_label_prediction.node_label_prediction_sklearn.ridge_classifier_cv_node_label_prediction import (
    RidgeClassifierCVNodeLabelPrediction,
)
from embiggen.edge_prediction.edge_prediction_sklearn.ridge_classifier_cv_edge_prediction import (
    RidgeClassifierCVEdgePrediction,
)
from embiggen.edge_label_prediction.edge_label_prediction_sklearn.ridge_classifier_cv_edge_label_prediction import (
    RidgeClassifierCVEdgeLabelPrediction,
)
...
def test_ridge_classifier_cv_node_label_prediction_smoke_test():
    ...
    for g, data in (
        ...
        model = RidgeClassifierCVNodeLabelPrediction().into_smoke_test()
        model.fit(g, node_features=data)
        model.predict(g, node_features=data)
        model.predict_proba(g, node_features=data)


def test_ridge_classifier_cv_edge_prediction_smoke_test():
    ...
    for g, data in (
        ...
        if data is None:
            data = DegreeSPINE().into_smoke_test().fit_transform(g)
        model = RidgeClassifierCVEdgePrediction().into_smoke_test()
...
def test_ridge_classifier_cv_edge_label_prediction_smoke_test():
    """Smoke test for the Ridge Classifier edge-label prediction model."""
    graph, data = get_words_data(Cora())
    ...
    for g, data in ((binary, None),):
        if data is None:
            data = DegreeSPINE().into_smoke_test().fit_transform(g)
        model = RidgeClassifierCVEdgeLabelPrediction().into_smoke_test()
        model.fit(g, node_features=data, edge_features=HyperSketching())
        model.predict(g, node_features=data, edge_features=HyperSketching())
        model.predict_proba(g, node_features=data, edge_features=HyperSketching())
